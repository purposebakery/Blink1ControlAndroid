	...
	public boolean WriteData(byte[] bytes) {
		try
		{
			...
			synchronized (_locker) {
				...
				//UsbInterface writeIntf = _usbDevice.getConfiguration(0).getInterface(0);
				UsbEndpoint writeEp = writeIntf.getEndpoint(1);
				...
				writeConnection.controlTransfer(UsbConstants.USB_DIR_OUT, )
				
				// Write the data as a bulk transfer with defined data length.
				int r = writeConnection.bulkTransfer(writeEp, bytes, bytes.length, 500);
				//int r = writeConnection.controlTransfer(UsbConstants.USB_DIR_OUT, 1, 0, 0, bytes, bytes.length, 500);
				if (r != -1) {
